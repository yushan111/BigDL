...
def get_masked_lm_output(bert_config, input_tensor, output_weights, positions,
  ...
  with tf.variable_scope("cls/predictions"):
    ...
    # scaled_loss = tf.nn.compute_average_loss(per_example_loss, 
    #                                          sample_weight=normalized_weights, global_batch_size=global_batch_size)
    # print_op = tf.print("get_masked_lm_output", scaled_loss, {"denominator": denominator, "label_weights": label_weights}, output_stream=sys.stdout)
...
def setup_distributed(urls, world_rank, world_size):
  ...
  # import logging 
  # logging.debug("Creating model with MultiWorkerMirroredStrategy")
  return strategy



def train_func(train_spec, eval_spec, model_fn, flag_dict, urls, world_rank, world_size, hparams):
  # from tensorflow.compat.v1.train import CheckpointSaverHook
  # import tensorflow.compat.v1 as tf
  # import tensorflow

  tf.logging.set_verbosity(tf.logging.INFO)
  # from distribution_utils import get_distribution_strategy
  tf.gfile.MakeDirs(flag_dict["output_dir"])
  ...
  #     # all_reduce_alg="nccl",
  #     num_packs=0)
  strategy = setup_distributed(urls, world_rank, world_size)
  print(f"****{tf.__file__}")
  ...
  print("*******start creating Estimator!!!")
...
def run(_):
  ...
  # train_func(train_input_fn, eval_input_fn, model_fn, flag_dict, urls, world_rank, world_size)
  # urls = ["localhost:12345"]
  urls = ["localhost:12345", "localhost:23456"]
  ...
  mllog.mlhtolog_end(key=mllog_constants.INIT_STOP)
  mllog.mllog_start(key=mllog_constants.RUN_START)

  result = [ray.remote(train_func).remote(train_spec, eval_spec, model_fn, flag_dict, urls=urls, world_rank=i, world_size=num_workers, hparams=hparams) for i in range(num_workers)]
...
def main(_):
  ...
  for input_file in input_files:
    tf.logging.info("  %s" % input_file)
  tf.logging.info(f"***********optimizer: {FLAGS.optimizer}")
  model_fn = model_fn_builder(
      bert_config=bert_config,
      init_checkpoint=FLAGS.init_checkpoint,
      learning_rate=FLAGS.learning_rate,
